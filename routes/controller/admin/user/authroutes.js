...
router.get("/register", (req, res) => {
  res.render("admin/register", { message: req.flash("message"), error: req.flash("error") });
});
...
router.post("/register", async (req, res) => {
  ...
  try {
      ...
      res.redirect("/admin.faces/register");
  }
});
...
router.post("/login", async (req, res) => {
  ...
  try {
      ...
      if (!utilisateur || !(await bcrypt.compare(motDePasse, utilisateur.motDePasse)) || !utilisateur.roles.includes('admin')) {
          req.flash("error", "Identifiants invalides");
          return res.redirect("/admin.faces/login");
      }
      ...
      res.redirect("/admin.faces/profile");
  } catch (error) {
      req.flash("error", "Erreur de connexion");
      res.redirect("/admin.faces/login");
  }
});
...
router.get("/update-profile", isAuthenticated, async (req, res) => {
  try {
      const user = await User.findById(req.user.id);
      res.render("admin/updateProfile", { user });
  } catch (error) {
      res.status(500).send("Erreur lors de la récupération du profil");
  }
});
...
router.get("/dashboard", isAuthenticated, async (req, res) => {
  res.render("admin/partials/dashboard", { message: req.flash("message"), error: req.flash("error") });
});
